...
class MqttService {
  ...
  Future<void> connect({String? host, int tcpPort = 1883, int wsPort = 9001}) async {
    ...
    try {
      if (chosenHost == null || chosenHost == 'localhost' || chosenHost == '127.0.0.1') {
        if (!kIsWeb) {
    ...
    Future<bool> tryConnect({required bool useWebSocket, required int port}) async {
      try {
        ...
        c.onConnected = () {
          _connected.add(true);
          _mqttLog.info('connected to ${useWebSocket ? 'ws' : 'tcp'}://$hostStr:$port');
        };
        c.onDisconnected = () {
          _connected.add(false);
          _mqttLog.info('disconnected from ${useWebSocket ? 'ws' : 'tcp'}://$hostStr:$port');
        };
        ...
        _client = c;
        _mqttLog.info('attempting ${useWebSocket ? 'WebSocket' : 'TCP'} connect to $hostStr:$port');
        try {
          ...
          _mqttLog.fine('connect attempt failed (${useWebSocket ? 'ws' : 'tcp'}): $e');
          try {
            _client?.disconnect();
          } catch (_) {}
        }
    ...
    connected = await tryConnect(useWebSocket: false, port: tcpPort);
    if (!connected) connected = await tryConnect(useWebSocket: true, port: wsPort);
  ...
  Future<bool> tryAttempt({required bool useWebSocket, required int port}) async {
      try {
        final c = MqttServerClient(hostStr, clientId);
        c.port = port;
        c.keepAlivePeriod = 20;
        ...
        if (c.connectionStatus?.state == MqttConnectionState.connected) {
          ...
          c.publishMessage(diagTopic, MqttQos.atLeastOnce, builder.payload!, retain: false);
          await Future.delayed(const Duration(milliseconds: 200));
          try {
            c.disconnect();
          } catch (_) {}
        ...
        _mqttLog.fine('testConnection attempt failed (${useWebSocket ? 'ws' : 'tcp'}): $e');
      }
      return false;
    }
